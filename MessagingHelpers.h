...
//////////////////////////////////// PROTOTYPES //////////////////////////////////////
void init_slot_freelist(void);                      // TEST03 ADD initialize slot free list
SlotPtr allocate_slot(void);                        // TEST03 ADD get free mail slot
void free_slot(SlotPtr s);                          // TEST03 ADD return slot to free list
void init_mailboxes(void);                          // TEST03 ADD initialize mailbox table
void init_proc_table(void);                         // TEST25 ADD initialize process messaging table

void waitq_push(WaitingProcessPtr* head, WaitingProcessPtr* tail, WaitingProcessPtr n);       // TEST05 ADD add process to wait queue
WaitingProcessPtr waitq_pop(WaitingProcessPtr* head, WaitingProcessPtr* tail);                // TEST05 ADD remove process from wait queue
